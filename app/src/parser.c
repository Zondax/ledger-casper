...
parser_error_t parser_validate(const parser_context_t *ctx) {
  ...
  for (uint8_t idx = 0; idx < numItems; idx++) {
    uint8_t pageCount = 0;
    CHECK_PARSER_ERR(parser_getItem((parser_context_t *)ctx, idx, tmpKey,
                                    sizeof(tmpKey), tmpVal, sizeof(tmpVal), 0,
                                    &pageCount))
  }
...
parser_error_t find_end_of_number(char *buffer, uint16_t bufferSize,
                                  uint16_t *output) {
  uint16_t index = 0;
  *output = 0;
...
parser_error_t inplace_insert_char(char *s, uint16_t sMaxLen, uint16_t pos,
                                   char separator) {
...
parser_error_t add_thousands_separators(char *buffer, uint16_t bufferSize,
                                        uint16_t *numsize) {
...
#define DISPLAY_RUNTIMEARG_AMOUNT_BIGNUM(CTX, LEN)                             \
  {                                                                            \
    ...
    bignumLittleEndian_to_bcd(bcdOut, bcdOutLen,                               \
                              (CTX)->buffer + (CTX)->offset + 1, (LEN)-1);     \
    ...
    CHECK_PARSER_ERR(                                                          \
        add_thousands_separators(buffer, sizeof(buffer), &numsize))            \
...
parser_error_t parser_display_runtimeArgMotes(uint8_t type, uint32_t dataLen,
...
parser_error_t parser_display_runtimeArg(uint8_t type, uint32_t dataLen,
                                         parser_context_t *ctx, char *outVal,
...
parser_error_t parser_runtimeargs_getData(const char *keystr, uint32_t *length,
                                          uint8_t *runtype, uint32_t num_items,
                                          parser_context_t *ctx) {
  char buffer[300];
...
parser_error_t parser_getItem_Transfer(ExecutableDeployItem item,
                                       parser_context_t *ctx,
...
parser_error_t parser_getItem_ModuleBytes(ExecutableDeployItem item,
                                          parser_context_t *ctx,
                                          uint8_t displayIdx, char *outKey,
                                          uint16_t outKeyLen, char *outVal,
                                          uint16_t outValLen, uint8_t pageIdx,
...
parser_error_t parser_getItemDeploy(ExecutableDeployItem item,
                                    parser_context_t *ctx, uint8_t displayIdx,
  ...
  switch (item.type) {
  case ModuleBytes: {
    return parser_getItem_ModuleBytes(item, ctx, displayIdx, outKey, outKeyLen,
  ...
  case Transfer: {
    return parser_getItem_Transfer(item, ctx, displayIdx, outKey, outKeyLen,
                                   outVal, outValLen, pageIdx, pageCount);
  }
  default: {
...
parser_error_t parser_getItem(parser_context_t *ctx, uint8_t displayIdx,
                              ...
                              uint8_t *pageCount) {
  if (ctx->tx_content == Deploy) {
    return _getItemDeploy(ctx, displayIdx, outKey, outKeyLen, outVal, outValLen,
                          pageIdx, pageCount);
  } else if (ctx->tx_content == TransactionV1) {
    return _getItemTxV1(ctx, displayIdx, outKey, outKeyLen, outVal, outValLen,
                        pageIdx, pageCount);
  } else {
    return parser_context_unknown_prefix;
  }
