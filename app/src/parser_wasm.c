...
static parser_error_t readHeader(parser_context_t *ctx,
                                 parser_tx_deploy_t *txObj) {
  txObj->header.pubkeytype = ctx->buffer[0];
  PARSER_ASSERT_OR_ERROR(txObj->header.pubkeytype == 0x01 ||
                             txObj->header.pubkeytype == 0x02,
                         parser_context_unknown_prefix);

  CHECK_PARSER_ERR(
      index_headerpart_deploy(txObj->header, header_deps, &ctx->offset));
  CHECK_PARSER_ERR(readU32(ctx, &txObj->header.lenDependencies));

  CHECK_PARSER_ERR(
      index_headerpart_deploy(txObj->header, header_chainname, &ctx->offset));
  ...
  if (txObj->payment.special_type == SystemPayment &&
      !txObj->payment.hasAmount) {
  ...
  if (txObj->session.type == ModuleBytes &&
      (err == parser_no_data || err == parser_unexpected_buffer_end)) {
    return parser_ok;
  }
...
parser_error_t parser_validate_wasm(const parser_context_t *ctx,
...
parser_error_t parser_getWasmItem(parser_context_t *ctx, uint8_t displayIdx,
  ...
  case 0:
    ...
    pageStringHex(outVal, outValLen,
                  (const char *)ctx->buffer +
                      header_length_deploy(ctx->tx_obj->header),
                  HASH_LENGTH, pageIdx, pageCount);
    return parser_ok;

  case 1:
    snprintf(outKey, outKeyLen, "BodyHash");
    CHECK_PARSER_ERR(index_headerpart_deploy(ctx->tx_obj->header,
                                             header_bodyhash, &ctx->offset));
    pageStringHex(outVal, outValLen, (const char *)ctx->buffer + ctx->offset,
                  HASH_LENGTH, pageIdx, pageCount);
    return parser_ok;
