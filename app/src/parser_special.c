...
#define CHECK_RUNTIME_ARGTYPE(CTX, NUM_ITEMS, STR, CONDITION)                  \
  {                                                                            \
    ...
    internal_type = 255;                                                       \
    CHECK_PARSER_ERR(                                                          \
        searchRuntimeArgs((STR), &type, &internal_type, (NUM_ITEMS), (CTX)));  \
...
#define COUNT_RUNTIME_ARGTYPE(CTX, NUM_ITEMS, STR, CONDITION)                  \
  {                                                                            \
    ...
    if (__err == parser_ok && (CONDITION))                                     \
      num_args_found += 1;                                                     \
    else if (__err != parser_runtimearg_notfound &&                            \
             __err != parser_unexpected_type)                                  \
      return __err;                                                            \
  }

parser_error_t render_fixed_delegation_items(
...
parser_error_t render_entry_point(parser_context_t *ctx, char *outKey,
...
parser_error_t parser_getItem_NativeTransfer(
  ...
  if (new_displayIdx >= item.UI_runtime_items ||
      displayIdx < item.UI_fixed_items) {
    return parser_unexpected_number_items;
  }
  ...
  if (item.with_generic_args > 0) {
    if (new_displayIdx == 0) {
      ...
      ctx->offset = start;
      CHECK_PARSER_ERR(showRuntimeArgsHash(item, ctx, len, name, name_len,
  ...
  if (!expected_items) {
    return showRuntimeArgByIndex(new_displayIdx, outKey, outKeyLen, outVal,
  ...
  if (!app_mode_expert()) {
    ...
    if (new_displayIdx == 2) {
      ...
      CHECK_PARSER_ERR(parser_runtimeargs_getData("id", &dataLength, &datatype,
                                                  num_items, ctx))
      return parser_display_runtimeArg(datatype, dataLength, ctx, outVal,
                                       outValLen, pageIdx, pageCount);
    }
...
parser_error_t checkNativeTransferArgs(parser_context_t *ctx,
                                       __Z_UNUSED ExecutableDeployItem *item,
                                       uint32_t num_items, uint32_t *fitems) {
  ...
  *fitems = 0;
  ...
  COUNT_RUNTIME_ARGTYPE(
      ctx, num_items, "id",
      ((type == TAG_OPTION && internal_type == TAG_U64) || type == TAG_U64))
  COUNT_RUNTIME_ARGTYPE(ctx, num_items, "target",
                        (type == TAG_KEY || type == TAG_UREF ||
                         type == TAG_BYTE_ARRAY || type == TAG_PUBLIC_KEY))
...
parser_error_t parseSystemPayment(parser_context_t *ctx,
  ...
  } else if (ret != parser_unexpected_number_items &&
             ret != parser_runtimearg_notfound &&
...
parser_error_t parser_getItem_SystemPayment(ExecutableDeployItem item,
  ...
  if (new_displayIdx == 0) {
    if (item.with_generic_args == 0) {
      ...
      CHECK_PARSER_ERR(parser_runtimeargs_getData(
          "amount", &dataLength, &datatype, item.UI_runtime_items, ctx))
      return parser_display_runtimeArgMotes(datatype, dataLength, ctx, outVal,
                                            outValLen, pageIdx, pageCount);
    } else {
...
parser_error_t render_fixed_delegation_items(
  ...
  case StoredContractByHash: {
    ...
    if (displayIdx == 2 && appExpertMode) {
      return render_entry_point(ctx, outKey, outKeyLen, outVal, outValLen,
                                pageIdx, pageCount);
    }
    break;
  }
...
parser_error_t parser_getItem_Delegation(ExecutableDeployItem *item,
  ...
  if (displayIdx < item->UI_fixed_items) {
    return render_fixed_delegation_items(item, ctx, displayIdx, outKey,
  ...
  if (item->with_generic_args) {
    ...
    if ((new_displayIdx == 0) && (item->UI_runtime_items == 2)) {
      ...
      uint8_t dtyp = 255;
      CHECK_PARSER_ERR(
          parser_runtimeargs_getData("amount", &dlen, &dtyp, dataLen, ctx))
      ...
      /*outVal, outValLen,*/
      /*pageIdx, pageCount);*/
      return parser_display_runtimeArgMotes(dtyp, dlen, ctx, outVal, outValLen,
                                            pageIdx, pageCount);
    }
  ...
  if (new_displayIdx == 0) {
    ...
    CHECK_PARSER_ERR(parser_runtimeargs_getData(
        "delegator", &dataLength, &datatype, item->UI_runtime_items, ctx))
    return parser_display_runtimeArg(datatype, dataLength, ctx, outVal,
                                     outValLen, pageIdx, pageCount);
  }
  if (item->special_type == ReDelegate) {
    if (new_displayIdx == 1) {
      ...
      CHECK_PARSER_ERR(parser_runtimeargs_getData(
          "validator", &dataLength, &datatype, item->UI_runtime_items, ctx))
      return parser_display_runtimeArg(datatype, dataLength, ctx, outVal,
                                       outValLen, pageIdx, pageCount);
    }
    if (new_displayIdx == 2) {
      ...
      CHECK_PARSER_ERR(parser_runtimeargs_getData(
          "new_validator", &dataLength, &datatype, item->UI_runtime_items, ctx))
      return parser_display_runtimeArg(datatype, dataLength, ctx, outVal,
                                       outValLen, pageIdx, pageCount);
    }
    if (new_displayIdx == 3) {
...
parser_error_t checkForDelegationItems(parser_context_t *ctx,
                                       ExecutableDeployItem *item,
                                       uint32_t num_items, bool redelegate,
                                       uint32_t *fitems) {
  uint8_t type = 0;
  uint8_t internal_type = 0;
  uint16_t num_args_found = 0;

  COUNT_RUNTIME_ARGTYPE(ctx, num_items, "amount",
                        (type == TAG_U512 || type == TAG_U32))
  ...
  *fitems = num_args_found;
  ...
  CHECK_RUNTIME_ARGTYPE(ctx, num_items, "amount",
                        type == TAG_U512 || type == TAG_U32);
  ...
  if (redelegate) {
    CHECK_RUNTIME_ARGTYPE(ctx, num_items, "new_validator",
                          type == TAG_PUBLIC_KEY);
...
parser_error_t parseDelegation(parser_context_t *ctx,
  ...
  if (item->type == ModuleBytes) {
    ...
    redelegation = (searchRuntimeArgs(("new_validator"), &type, &internal_type,
                                      (num_items), (ctx)) == parser_ok);

    uint32_t dataLength = 0;
    // this should be present in any transaction of this type
    CHECK_PARSER_ERR(parser_runtimeargs_getData("auction", &dataLength, &type,
                                                num_items, ctx));
    PARSER_ASSERT_OR_ERROR(type == TAG_STRING, parser_unexpected_type);

    char buffer[100] = {0};
    PARSER_ASSERT_OR_ERROR(dataLength <= sizeof(buffer) &&
                               ctx->bufferLen >= ctx->offset + dataLength,
    ...
    if (stringLength == strlen(DELEGATE_STR) &&
        strncmp(buffer, DELEGATE_STR, stringLength) == 0) {
  ...
  const parser_error_t err =
      checkForDelegationItems(ctx, item, num_items, redelegation, &found_items);
  ...
  if (err == parser_runtimearg_notfound || err == parser_unexpected_type) {
    ...
    const parser_error_t add_amount_err =
        searchRuntimeArgs("amount", &type, &internal_type, num_items, ctx);
  ...
  if (app_mode_expert()) {
    ...
    item->UI_fixed_items = 2;
    uint8_t has_version = item->type == StoredVersionedContractByHash ||
                                  item->type == StoredVersionedContractByName
                              ? 1
                              : 0;
