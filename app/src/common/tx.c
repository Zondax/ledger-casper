...
const char *tx_parse_message() {
  const uint8_t err = parser_parse_message(&ctx_parsed_tx, tx_get_buffer(),
...
zxerr_t tx_getMessageItem(int8_t displayIdx, char *outKey, uint16_t outKeyLen,
                          char *outVal, uint16_t outValLen, uint8_t pageIdx,
                          uint8_t *pageCount) {
  ...
  const parser_error_t err =
      parser_getMessageItem(&ctx_parsed_tx, displayIdx, outKey, outKeyLen,
...
zxerr_t tx_parse_wasm() {
  const parser_error_t err = parser_parse_wasm(&ctx_parsed_tx, tx_get_buffer(),
                                               tx_get_buffer_length());
...
zxerr_t tx_validate_wasm() {
  const parser_error_t err =
      parser_validate_wasm(&ctx_parsed_tx, ctx_parsed_tx.tx_obj);
  return (err == parser_ok) ? zxerr_ok : zxerr_unknown;
}
...
zxerr_t tx_getWasmItem(int8_t displayIdx, char *outKey, uint16_t outKeyLen,
                       char *outVal, uint16_t outValLen, uint8_t pageIdx,
                       uint8_t *pageCount) {

  const parser_error_t err =
      parser_getWasmItem(&ctx_parsed_tx, displayIdx, outKey, outKeyLen, outVal,
                         outValLen, pageIdx, pageCount);
