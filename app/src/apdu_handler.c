...
__Z_INLINE void handleSignWasm(volatile uint32_t *flags, volatile uint32_t *tx,
                               uint32_t rx) {
  wasm_counter++;
...
__Z_INLINE void handleGetAddr(volatile uint32_t *flags, volatile uint32_t *tx,
                              uint32_t rx) {
  extractHDPath(rx, OFFSET_DATA);
...
__Z_INLINE void handleSignMessage(volatile uint32_t *flags,
  ...
  if (error_msg != NULL) {
    const size_t error_msg_length =
        strnlen(error_msg, sizeof(G_io_apdu_buffer));
    MEMCPY(G_io_apdu_buffer, error_msg, error_msg_length);
    *tx += (error_msg_length);
    THROW(APDU_CODE_DATA_INVALID);
  }
