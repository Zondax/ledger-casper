...
uint16_t header_length_deploy(parser_header_deploy_t header) {
  uint16_t pubkeyLen =
      1 + (header.pubkeytype == 0x02 ? SECP256K1_PK_LEN : ED25519_PK_LEN);
...
parser_error_t index_headerpart_deploy(parser_header_deploy_t head,
  ...
  *index = 0;
  uint16_t pubkeyLen =
      1 + (head.pubkeytype == 0x02 ? SECP256K1_PK_LEN : ED25519_PK_LEN);
...
parser_error_t parseModuleBytes(parser_context_t *ctx,
  ...
  if (item->phase == Payment) {
    CHECK_PARSER_ERR(parseSystemPayment(
        ctx, item, deploy_argLen)); // only support for system payment
...
parser_error_t parseTransfer(parser_context_t *ctx,
                             ExecutableDeployItem *item) {
  uint32_t start = *(uint32_t *)&ctx->offset;
  uint32_t deploy_argLen = 0;
...
parser_error_t check_entrypoint(parser_context_t *ctx,
                                ExecutableDeployItem *item,
                                uint32_t *num_runs) {
  char buffer[100] = {0};
...
parser_error_t parseStoredContractByHash(parser_context_t *ctx,
                                         ExecutableDeployItem *item) {
  uint32_t start = *(uint32_t *)&ctx->offset;
...
parser_error_t parseStoredContractByName(parser_context_t *ctx,
                                         ExecutableDeployItem *item) {
  uint32_t start = *(uint32_t *)&ctx->offset;
  CHECK_PARSER_ERR(parse_item(ctx));
...
parser_error_t parser_read_deploy(parser_context_t *ctx,
                                  parser_tx_deploy_t *v) {
  v->header.pubkeytype = ctx->buffer[0];
  PARSER_ASSERT_OR_ERROR(v->header.pubkeytype == 0x01 ||
                             v->header.pubkeytype == 0x02,
                         parser_context_unknown_prefix);

  CHECK_PARSER_ERR(
      index_headerpart_deploy(v->header, header_deps, &ctx->offset));
  CHECK_PARSER_ERR(_readUInt32(ctx, &v->header.lenDependencies));

  CHECK_PARSER_ERR(
      index_headerpart_deploy(v->header, header_chainname, &ctx->offset));
...
parser_error_t _validateTxDeploy(const parser_context_t *c,
  ...
  PARSER_ASSERT_OR_ERROR(MEMCMP(hash, c->buffer + index, BLAKE2B_256_SIZE) == 0,
                         parser_context_mismatch);

  return parser_ok;
}

uint8_t _getNumItemsDeploy(__Z_UNUSED const parser_context_t *c,
                           const parser_tx_deploy_t *v) {
  uint8_t basicnum = app_mode_expert() ? 9 : 4;
  uint8_t itemCount =
      basicnum + v->payment.UI_fixed_items + v->payment.UI_runtime_items +
      v->session.UI_fixed_items +
      v->session
          .UI_runtime_items; // header + payment + session v->session.num_items
  return itemCount;
}
...
parser_error_t _getItemDeploy(parser_context_t *ctx, uint8_t displayIdx,
  ...
  *pageCount = 1;
  ...
  if (displayIdx == 0) {
    ...
    return parser_printBytes((const uint8_t *)(ctx->buffer + ctx->offset), 32,
  ...
  if (displayIdx == 2) {
    CHECK_PARSER_ERR(index_headerpart_deploy(parser_tx_obj.header,
                                             header_chainname, &ctx->offset));
    DISPLAY_STRING("Chain ID", ctx->buffer + 4 + ctx->offset,
                   parser_tx_obj.header.lenChainName)
  }

  if (displayIdx == 3) {
    snprintf(outKey, outKeyLen, "Account");
    CHECK_PARSER_ERR(index_headerpart_deploy(parser_tx_obj.header,
                                             header_pubkey, &ctx->offset));
    uint16_t pubkeyLen =
        1 + (parser_tx_obj.header.pubkeytype == 0x02 ? SECP256K1_PK_LEN
                                                     : ED25519_PK_LEN);
    return parser_printAddress((const uint8_t *)(ctx->buffer + ctx->offset),
                               pubkeyLen, outVal, outValLen, pageIdx,
                               pageCount);
  }
  ...
  if (app_mode_expert()) {
    ...
    if (displayIdx == 5) {
      ...
      CHECK_PARSER_ERR(index_headerpart_deploy(parser_tx_obj.header, header_ttl,
                                               &ctx->offset));
      uint64_t value = 0;
  ...
  uint16_t total_payment_items = parser_tx_obj.payment.UI_fixed_items +
                                 parser_tx_obj.payment.UI_runtime_items;
  if (new_displayIdx < total_payment_items) {
    if (parser_tx_obj.payment.special_type == SystemPayment) {
      return parser_getItem_SystemPayment(
          parser_tx_obj.payment, ctx, new_displayIdx, outKey, outKeyLen, outVal,
  ...
  uint16_t total_session_items = parser_tx_obj.session.UI_fixed_items +
                                 parser_tx_obj.session.UI_runtime_items;
  ...
  if (new_displayIdx < total_session_items) {
    ...
    if (special_type == NativeTransfer) {
      return parser_getItem_NativeTransfer(
          parser_tx_obj.session, ctx, new_displayIdx, outKey, outKeyLen, outVal,
          outValLen, pageIdx, pageCount);
    } else if (special_type == Delegate || special_type == UnDelegate ||
               special_type == ReDelegate || special_type == Generic) {
      return parser_getItem_Delegation(&parser_tx_obj.session, ctx,
                                       new_displayIdx, outKey, outKeyLen,
                                       outVal, outValLen, pageIdx, pageCount);
    } else {
      return parser_unexpected_type;
    }
