...
static bool get_next_hash_bit(char *hash_input, uint8_t *index,
                              uint8_t *offset);
...
zxerr_t crypto_extractPublicKey(const uint32_t path[HDPATH_LEN_DEFAULT],
  ...
  CATCH_CXERROR(os_derive_bip32_with_seed_no_throw(
      HDW_NORMAL, CX_CURVE_256K1, path, HDPATH_LEN_DEFAULT, privateKeyData,
      NULL, NULL, 0));
...
zxerr_t pubkey_to_hash(const uint8_t *pubkey, uint16_t pubkeyLen,
                       uint8_t *out) {
...
zxerr_t crypto_sign(uint8_t *signature, uint16_t signatureMaxlen,
  ...
  if (signature == NULL || message == NULL || sigSize == NULL ||
      signatureMaxlen < sizeof(signature_t)) {
    return zxerr_unknown;
  }
  ...
  case Transaction: {
    const uint8_t *message_digest =
        message + header_length_deploy(parser_tx_obj.header);
  ...
  *sigSize = 0;
  ...
  CATCH_CXERROR(cx_ecdsa_sign_no_throw(
      &cx_privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256, hash, CX_SHA256_SIZE,
      signature_object->der_signature, &signatureLength, &tmpInfo));

  const err_convert_e err_c = convertDERtoRSV(
      signature_object->der_signature, tmpInfo, signature_object->r,
...
zxerr_t crypto_hashChunk(const uint8_t *buffer, uint32_t bufferLen,
  ...
  if ((operation == hash_update && buffer == NULL) ||
      (operation == hash_finish &&
       (output == NULL || outputLen < CX_SHA256_SIZE))) {
  ...
  case hash_start:
    CHECK_CX_OK(
        cx_blake2b_init2_no_throw(&body_hash_ctx, 256, NULL, 0, NULL, 0));
    break;
...
zxerr_t crypto_extractPublicKey(const uint32_t path[HDPATH_LEN_DEFAULT],
                                uint8_t *pubKey, uint16_t pubKeyLen) {
  const char *tmp = "7f747b67bd3fe63c2a736739dfe40156d622347346e70f68f51c178a75"
                    "ce5537a087c03779";
...
zxerr_t crypto_fillAddress(uint8_t *buffer, uint16_t buffer_len,
  ...
  zxerr_t err = crypto_extractPublicKey(hdPath, answer->publicKey,
  ...
  err = encode_addr((char *)addr_plus_prefix, SECP256K1_PK_LEN + 1,
                    (char *)answer->address);
  ...
  *addrLen =
      sizeof_field(answer_t, address) + sizeof_field(answer_t, publicKey);
  return zxerr_ok;
}

zxerr_t encode_addr(char *address, const uint8_t addressLen,
                    char *encodedAddr) {
...
zxerr_t encode(char *address, const uint8_t addressLen, char *encodedAddr) {
  ...
  for (int i = 0; i < nibblesLen; i++) {
    ...
    if (is_alphabetic(c)) {
      get_next_hash_bit((char *)hash_input, &index, &offset)
          ? to_uppercase((uint8_t *)&c)
...
zxerr_t encode_hex(char *bytes, const uint8_t bytesLen, char *output,
  ...
  for (int i = 0; i < nibblesLen; i++) {
    ...
    char c = HEX_CHARS[char_index];
    if (is_alphabetic(c)) {
      get_next_hash_bit(bytes, &index, &offset) ? to_uppercase((uint8_t *)&c)
                                                : to_lowercase((uint8_t *)&c);
    }
    output[i] = c;
  }
